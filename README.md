...
1. **카나리아 조회** — 이 API는 실패해도 에러가 아니라 `data:false`를 주기
   때문에, "진짜 만석"과 "뭔가 조용히 고장남"을 구분할 기준이 필요하다.
   그래서 항상 열려 있을 만한 날짜(오늘+55일, `CANARY_OFFSET_DAYS`)를 먼저
   확인해서, 그것도 `false`면 지금 결과를 못 믿는다는 뜻으로 보고 목표일
   조회 자체를 건너뛰고 "신뢰 불가" 알림을 보낸다. 고정 날짜가 아니라 매번
   "오늘부터 N일 뒤"로 계산하므로 날짜가 손으로 관리할 필요 없이 계속
   미래로 굴러간다 (예약 가능 범위 끝자락은 아직 아무도 안 채워서 대체로
   비어있다는 걸 실측으로 확인함).
...
`.github/workflows/check.yml`의 `env: TARGET_DATE` 값을 고쳐서 push하면 된다
(Cloudflare 재배포 불필요). 예약 가능 범위는 오늘+60일이므로 그 안이어야 함.

카나리아 날짜는 손으로 안 바꿔도 된다 (위 참고). 그래도 "신뢰 불가" 알림이
계속 오면, 예약 가능 범위 끝자락까지 실제로 다 찼거나(수요 폭증) 환경이
막힌 것이니 `scripts/check.mjs`의 `CANARY_OFFSET_DAYS`를 더 크게(예: 58)
조정해볼 것.
